...
async function main() {
  ...
  console.log(`[INFO] Migraciones aplicadas: ${appliedSet.size}`);
  ...
  console.log(`[INFO] Migraciones en filesystem: ${all.length}`);

  // 3. Encontrar las pendientes
  const pending = all.filter((m) => !appliedSet.has(m));
  console.log(`[INFO] Migraciones pendientes (FS no aplicadas): ${pending.length}`);
  for (const m of pending) {
    console.log(`  - ${m}`);
  }

  // 4. Marcar como aplicadas con SQL directo (idempotente, NO ejecuta la migración)
  console.log(`\n[ACTION] Marcando ${pending.length} migraciones como aplicadas...`);
