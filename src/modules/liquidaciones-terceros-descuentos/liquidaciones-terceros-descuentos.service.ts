...
export const LiquidacionesTercerosDescuentosService = {
  ...
  async guardarPropietarios(
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 3) Sincronizar la marca de multi-propietario del cierre.
    ...
    // 4) Recalcular impuestos prorrateados por copropietario (si aplica).
    ...
    // 5) Recalcular totales del cierre (puede haber cambiado total_impuestos).
    await this.recalcularTotales(liquidacionTerceroFinalId);
    ...
    return await this.obtenerPropietarios(liquidacionTerceroFinalId);
  },
  ...
  ///   (una por copropietario) con `valor_total = global × (porcentaje/100)`
  ///   y `base_calculo = base_global × (porcentaje/100)`.
  ...
  async recalcularImpuestosPorPropietario(liquidacionTerceroFinalId: string) {
    const cierre = await prisma.liquidacion_tercero_final.findUnique({
      where: { id: liquidacionTerceroFinalId },
      select: { id: true, estado: true, es_multi_propietario: true, adicionales: true },
    });
    if (!cierre) throw new Error('Liquidación final de tercero no encontrada');
    ...
    // Base imponible por copropietario = V.LIQUIDAR × pct − descuentos_globales_prop
    // (es decir, el VALOR A FACTURAR de cada prop). Es la base sobre la que
    // se aplican los impuestos / retenciones.
    const vlCierre = await prisma.liquidacion_tercero_final.findUnique({
      where: { id: liquidacionTerceroFinalId },
      select: { valor_liquidar: true },
    });
    const valorLiqTotal = toNumber(vlCierre?.valor_liquidar || 0);
    const conceptosCierre = await prisma.liquidacion_tercero_final_concepto.findMany({
      where: { liquidacion_tercero_final_id: liquidacionTerceroFinalId, deleted_at: null },
    });
    ...
    for (const c of conceptosCierre) {
      if (c.tipo === 'IMPUESTO') continue;
      if (sumByCat[c.tipo] === undefined) continue;
      sumByCat[c.tipo] += toNumber(c.valor_total);
    }
    const basePorProp: Record<string, number> = {};
    for (const p of propietariosActivos) {
      const pct = toNumber(p.porcentaje);
      const factor = pct / 100;
      const descGlob =
        sumByCat.COSTO_LABORAL * factor +
        sumByCat.GASTO_OPERATIVO * factor +
        sumByCat.ANTICIPO * factor;
      const vlProp = valorLiqTotal * factor;
      basePorProp[p.id] = Math.max(0, vlProp - descGlob);
    }
    ...
    // 4) Modo multi-propietario: N filas por impuesto, una por copropietario.
    ...
    // Pre-computar el valor de RETENCION_ICA por propietario para usarlo
    // como base de AVISOS_TABLEROS y SOBRETASA_BOMBERIL (regla de dominio:
    // ambos gravan sobre el valor de RETENCION_ICA ya calculado, NO sobre
    // el VALOR A FACTURAR del prop).
    const retIcaGlobal = impuestosGlobales.find((ig) => ig.concepto === 'RETENCION_ICA');
    const retIcaPorProp: Record<string, number> = {};
    if (retIcaGlobal) {
      for (const prop of propietariosActivos) {
        const pct = toNumber(prop.porcentaje);
        retIcaPorProp[prop.id] = retIcaGlobal.valor_total * (pct / 100);
      }
    }

    for (const ig of impuestosGlobales) {
      for (const prop of propietariosActivos) {
        const key = `${ig.concepto}|${prop.id}`;
        if (manualKeys.has(key)) continue;  // respetar edición manual
        const pct = toNumber(prop.porcentaje);
        let baseProp: number;
        if (
          ig.concepto === 'AVISOS_TABLEROS' ||
          ig.concepto === 'SOBRETASA_BOMBERIL'
        ) {
          // REGLA DE DOMINIO: la base es el valor de RETENCION_ICA ya
          // calculado para ESTE propietario (no el VALOR A FACTURAR).
          // Ej: para Nelly (15%) con retICA global = $90.685 → base =
          // $13.603 (no $1.360.275).
          baseProp =
            retIcaPorProp[prop.id] ?? ig.base_calculo * (pct / 100);
        } else {
          // RETENCION_ICA / RETENCION_FUENTE: VALOR A FACTURAR del prop
          // (post-desc-globales). Si no se pudo calcular (legacy),
          // fallback al V.LIQUIDAR proporcional.
          baseProp = basePorProp[prop.id] ?? ig.base_calculo * (pct / 100);
        }
        // El valor del impuesto por prop se calcula sobre SU propia base.
        const valorProp = baseProp * (ig.porcentaje / 100);
        filasNuevas.push({
          id: randomUUID(),
          liquidacion_tercero_final_id: liquidacionTerceroFinalId,
          tipo: 'IMPUESTO',
          ...
          valor_unitario: 0,
          porcentaje: ig.porcentaje,
          valor_total: valorProp,
          base_calculo: baseProp,
          calculado: true,
