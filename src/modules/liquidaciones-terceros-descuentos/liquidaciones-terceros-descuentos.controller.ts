...
export class LiquidacionesTercerosDescuentosController {
  ...
  static async guardarPropietarios(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      // Cambia la GEOMETRÍA de la hoja (aparece/desaparece la sección de
      // copropietarios y las filas de impuesto se multiplican por N), así
      // que no se puede difundir como patch de celda: el resto del room
      // tiene que releer.
      emitSheetInvalidate({
        scope: 'cierres-finales',
        anio: (result as any).anio,
        mes: (result as any).mes,
        cierreId: id,
        accion: 'propietarios',
      });
