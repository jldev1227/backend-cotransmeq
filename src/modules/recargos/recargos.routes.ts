...
export async function recargosRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    "/recargos/tipos-recargo/activos",
    RecargosController.obtenerTiposRecargo,
  );

  // Obtener estadísticas (debe ir antes de /:id)
  fastify.get(
    "/recargos/estadisticas/resumen",
    RecargosController.obtenerEstadisticas,
  );
  ...
  fastify.post(
    "/recargos/eliminar-multiple",
    RecargosController.eliminarMultiple,
  );

  // Cambiar estado de múltiples recargos
  fastify.patch(
    "/recargos/cambiar-estado-multiple",
    RecargosController.cambiarEstadoMultiple,
  );
