...
export const LiquidacionesService = {
  ...
  async obtenerTodas(filters?: {
    ...
    const [liquidaciones, total, totalPendientes, montoTotalAgg] =
      await Promise.all([
  ...
  async previewRecargos(
    ...
    const valorHoraBase = configSalarial
      ? Number(configSalarial.salario_basico) /
        configSalarial.horas_mensuales_base
      : 0;
    const pagaFestivos = configSalarial?.paga_dias_festivos ?? false;
    const porcentajeFestivos = configSalarial
      ? Number(configSalarial.porcentaje_festivos)
      : 75;
    ...
    const planillasDetalle = planillas.map((planilla) => {
      ...
      if (pagaFestivos) {
        const diasFestivosP = diasDetalle.filter(
          (d) => !d.disponibilidad && (d.es_festivo || d.es_domingo),
        ).length;
        planillaFestivos = Math.round(
          diasFestivosP * valorHoraBase * (porcentajeFestivos / 100) * 10,
        );
      }
    ...
    let totalFestivos = 0;
    if (pagaFestivos) {
      const diasFestivos = planillasDetalle.reduce(
        (count, p) =>
          count +
          p.dias.filter(
            (d) => !d.disponibilidad && (d.es_festivo || d.es_domingo),
      ...
      totalFestivos =
        diasFestivos * valorHoraBase * (porcentajeFestivos / 100) * 10; // 10 horas base festivo
    }
